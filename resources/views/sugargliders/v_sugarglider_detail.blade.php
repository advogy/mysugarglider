            ...
            <div class="profile-card">
                <div class="profile-img-wrapper">
                    @if ($collection->sgGambar)
                        <button type="button" onclick="previewPhoto('{{ asset('/upload/sugargliders/' . $collection->sgGambar) }}', '{{ addslashes($collection->sgNama) }}')" class="w-full h-full block focus:outline-none cursor-zoom-in">
                            <img src="{{ asset('/upload/sugargliders/' . $collection->sgGambar) }}" alt="{{ $collection->sgNama }}" class="profile-img hover:opacity-90 transition-opacity">
                        </button>
                    @else
                ...
                <div class="profile-info">
                    ...
                    @if ($collection->sgKeterangan)
                        <p class="text-bark-muted text-sm italic mb-4">"{{ $collection->sgKeterangan }}"</p>
                    @endif
                ...
                <div class="space-y-3">
                    @foreach ($keturunans as $kt)
                        <a href="{{ route('sugarglider.show', $kt->id) }}" class="block p-4 rounded-xl hover:bg-gray-50 transition-colors border border-gray-100">
                            <div class="font-bold text-bark text-lg">{{ $kt->nama }}</div>
                            <div class="text-bark-muted text-sm">{{ $kt->jenis }}</div>
                        </a>
                    @endforeach
                </div>
            </div>
                ...
                <div class="info-grid">
                    <div class="info-item">
                        ...
                        <div class="info-value">
                            @include('partials.gender', ['kelamin' => $collection->sgKelamin])
                        </div>
                    </div>
                    <div class="info-item">
                        <div class="info-label">Usia</div>
                        <div class="info-value">{{ $usia ?? '—' }}</div>
                    </div>
                    <div class="info-item">
                ...
                <div class="grid sm:grid-cols-2 gap-6 mb-8">
                    <div class="pedigree-box male-box">
                        ...
                        @if ($collection->sgIndukanJantan && $indukan->mId)
                            <a href="{{ route('sugarglider.show', $indukan->mId) }}" class="font-extrabold text-xl text-bark hover:underline">{{ $indukan->jantan }}</a>
                            <p class="text-bark-muted text-sm mt-1">{{ $indukan->mJenis }}</p>
                        @else
                            <p class="text-bark-muted italic">Tidak diketahui</p>
                        @endif
                    </div>
                    <div class="pedigree-box female-box">
                        ...
                        @if ($collection->sgIndukanBetina && $indukan->fId)
                            <a href="{{ route('sugarglider.show', $indukan->fId) }}" class="font-extrabold text-xl text-bark hover:underline">{{ $indukan->betina }}</a>
                            <p class="text-bark-muted text-sm mt-1">{{ $indukan->fJenis }}</p>
                        @else
                            <p class="text-bark-muted italic">Tidak diketahui</p>
                        @endif
                    </div>
