...
{{-- ═══════ SIDEBAR ═══════ --}}
<aside id="sidebar"
       class="sidebar translate-x-0 xl:translate-x-0 scrollbar-thin overflow-y-auto">
    ...
    <div class="flex items-center justify-between px-5 py-5 border-b border-white/10 flex-shrink-0">
        ...
        <button id="sidebar-close"
                class="xl:hidden text-white/50 hover:text-white transition-colors">
    ...
    <nav class="flex-1 px-3 py-4 space-y-0.5">
        ...
        <a href="{{ route('shelter.index') }}"
           class="sidebar-link {{ request()->is('*shelters*') ? 'active' : '' }}">
            <i class="bi bi-house-heart-fill text-base"></i>
            <span>{{ __('text.shelter_data') }}</span>
        </a>

        <a href="{{ route('sugarglider.index') }}"
           class="sidebar-link {{ request()->is('*sugargliders*') ? 'active' : '' }}">
            <i class="bi bi-heart-fill text-base"></i>
            <span>{{ __('text.sugarglider_data') }}</span>
        </a>

        <a href="{{ route('collection.index') }}"
           class="sidebar-link {{ request()->is('*collections*') ? 'active' : '' }}">
            <i class="bi bi-collection-fill text-base"></i>
            <span>{{ __('text.collection_data') }}</span>
        </a>

        <a href="{{ route('adoption.index') }}"
           class="sidebar-link {{ request()->is('*adoptions*') ? 'active' : '' }}">
            <i class="bi bi-journal-check text-base"></i>
            <span>{{ __('text.adoption_data') }}</span>
        </a>

        <p class="sidebar-title">Jelajahi</p>

        <a href="{{ route('pedigree.index') }}"
           class="sidebar-link {{ request()->is('*pedigree*') ? 'active' : '' }}">
            <i class="bi bi-diagram-3-fill text-base"></i>
            <span>{{ __('text.pedigree') }}</span>
        </a>

        <a href="{{ route('adoption.list') }}"
           class="sidebar-link {{ request()->routeIs('adoption.list') ? 'active' : '' }}">
            <i class="bi bi-heart-arrow text-base"></i>
...
<div id="sidebar-overlay"
     class="fixed inset-0 bg-bark/50 z-30 xl:hidden hidden backdrop-blur-sm"
...
<div class="xl:pl-64 min-h-screen flex flex-col transition-all duration-300">
    ...
    <header class="topbar">
        ...
        <button id="sidebar-open"
                class="xl:hidden p-2 rounded-xl hover:bg-cream transition-colors"
                onclick="openSidebar()">
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <span class="hidden sm:block text-sm font-bold text-bark">
                    {{ Auth::user()->name }}
                </span>
            </div>
        </div>
    ...
    <footer class="px-6 py-4 border-t border-cream-dark flex flex-col sm:flex-row items-center justify-between gap-2">
        ...
        <p class="text-bark-muted text-xs">
            ...
            <a href="https://athoria.me" target="_blank" rel="noopener"
               class="text-sage font-semibold hover:underline">AthoRia.me</a>
        </p>
    </footer>
</div>
...
// Init: on small screens sidebar is hidden
if (window.innerWidth < 1280) {
    document.getElementById('sidebar').classList.add('-translate-x-full');
}
</script>
