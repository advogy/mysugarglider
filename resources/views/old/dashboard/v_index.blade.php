...
{{-- Greeting --}}
<div class="mb-8">
    <h2 class="text-2xl font-display font-bold text-bark">
        Halo, {{ Auth::user()->name }} 👋
    </h2>
    <p class="text-bark-muted text-sm mt-1">
        Berikut ringkasan data sugar glider Anda hari ini.
    </p>
</div>
...
<div class="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-8">
    <x-stat-widget
        label="Kandang"
        :value="$count_shelters"
        icon="bi-house-heart-fill"
        color="sage"
        :url="route('shelter.index')"
    />
    <x-stat-widget
        label="Sugar Glider"
        :value="$count_sugargliders"
        icon="bi-heart-fill"
        color="sky"
        :url="route('sugarglider.index')"
    />
    <x-stat-widget
        label="Koleksi"
        :value="$count_collections"
    ...
    <x-stat-widget
        label="Siap Diadopsi"
        :value="$count_adoptions"
        icon="bi-journal-check"
        color="sage"
        :url="route('adoption.index')"
    />
    <x-stat-widget
        label="Dapat Diadopsi"
        :value="$count_adoptable"
        icon="bi-heart-arrow"
        color="pink"
        ...
        <div class="space-y-3">
            @php
                $steps = [
                    ...
                    ['num'=>2, 'label'=>'Tambahkan kandang',     'route'=>route('shelter.index'),     'icon'=>'bi-house-heart-fill',    'done'=>$count_shelters > 0],
                    ['num'=>3, 'label'=>'Input data Sugar Glider','route'=>route('sugarglider.index'),'icon'=>'bi-heart-fill',          'done'=>$count_sugargliders > 0],
                    ['num'=>4, 'label'=>'Buat koleksi',          'route'=>route('collection.index'),  'icon'=>'bi-collection-fill',     'done'=>$count_collections > 0],
                    ['num'=>5, 'label'=>'Kelola adopsi',         'route'=>route('adoption.index'),    'icon'=>'bi-journal-check',       'done'=>$count_adoptions > 0],
                ];
            @endphp
            @foreach ($steps as $step)
                <a href="{{ $step['route'] }}"
                   class="flex items-center gap-4 p-4 rounded-2xl border-2 transition-all duration-200
                          {{ $step['done']
                              ? 'border-sage/20 bg-sage-50'
                              : 'border-cream-dark hover:border-sage/30 hover:bg-cream' }}">
                    ...
                    <div class="flex-1">
                        <p class="font-semibold text-bark text-sm {{ $step['done'] ? 'line-through text-bark-muted' : '' }}">
                            {{ $step['label'] }}
                        </p>
                    </div>
    ...
    <div class="space-y-4">
        ...
        <div class="be-card p-5">
            <h4 class="font-bold text-bark text-sm mb-4 flex items-center gap-2">
                <i class="bi bi-lightning-fill text-honey-dark"></i>
                Akses Cepat
            </h4>
            <div class="space-y-2">
                <a href="{{ route('sugarglider.create') }}"
                   class="flex items-center gap-3 text-sm font-semibold text-bark-light hover:text-sage transition-colors">
